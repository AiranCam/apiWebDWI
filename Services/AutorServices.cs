...
namespace WebApi92.Services
{
    public class AutorServices : IAutorServices
    {
        ...
        public async Task<Response<Autor>> GetAutorById(int id)
        {
            try
            {
                Autor result = (await _context.Database.GetDbConnection().QueryAsync<Autor>("GetAutorId ", new {PkAutor = id }, commandType: CommandType.StoredProcedure)).FirstOrDefault();
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception("Hay un error pa" + ex.Message);
            }
        }
        ...
        public async Task<Response<Autor>> CreateAutor(Autor request)
        {
            ...
            {
                throw new Exception("Hay un error pa");
            }
        }
        ...
        public async Task<Response<Autor>> EliminarAutor(int id)
        {
            try
            {
                Autor result = (await _context.Database.GetDbConnection().QueryAsync<Autor>("spDeleteAutor", new { PkAutor = id }, commandType: CommandType.StoredProcedure)).FirstOrDefault();
